...
python initialize-config.py $1 > newconfig.ini
set -a               # turn on automatic export
source newconfig.ini   # execute all commands in the file
set +a 

# Create TFRecord from images + csv file on blob storage
echo "Creating TF Record"
python write_tf_record.py $1

# # Download tf model if it does not exist
...
# #Initially the below didn't work on Mac OS. Added .bak as reccommeded
# # here https://stackoverflow.com/questions/4247068/sed-command-with-i-option-failing-on-mac-but-works-on-linux
...
# sed -i .bak "s/${old_train_path//\//\\/}/${tf_train_record//\//\\/}/g" $temp_pipeline
# sed -i .bak "s/${old_train_path//\//\\/}/${tf_train_record//\//\\/}/g" $temp_pipeline
# sed -i .bak "s/${old_val_path//\//\\/}/${tf_val_record//\//\\/}/g" $temp_pipeline
# sed -i .bak "s/${old_checkpoint_path//\//\\/}/${fine_tune_checkpoint//\//\\/}/g" $temp_pipeline
# sed -i .bak "s/$num_steps_marker[[:space:]]*[[:digit:]]*/$num_steps_marker $train_iterations/g" $temp_pipeline
# sed -i .bak "s/$num_examples_marker[[:space:]]*[[:digit:]]*/$num_examples_marker $eval_iterations/g" $temp_pipeline
# sed -i .bak "s/$num_classes_marker[[:space:]]*[[:digit:]]*/$num_classes_marker $num_classes/g" $temp_pipeline


# Train model on TFRecord
# echo "Training model"
# rm -rf $train_dir
# echo "Running ${tf_location_legacy}/train.py --train_dir=$train_dir --pipeline_config_path=$temp_pipeline --logtostderr"
...
# echo "python ${tf_location}/export_inference_graph.py --input_type 'image_tensor' --pipeline_config_path $temp_pipeline --trained_checkpoint_prefix ${train_dir}/model.ckpt-$train_iterations --output_directory $inference_output_dir"
...
python ${python_file_directory}/create_predictions_v2.py newconfig.ini
